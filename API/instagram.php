...
class instagramPhp{
 ...
 public function simpleDisplay($shots){
 	...
 	public function simpleDisplayForm($shots){
        $simpleDisplay = '';
        if(!empty($shots->data)){
            $simpleDisplay.='<div class="instagram_shots">';
            	$i = 0;
                foreach($shots->data as $istg){
                    //The image
                    $istg_thumbnail = $istg->{'images'}->{'thumbnail'}->{'url'}; //Thumbnail
                    $istg_standard = $istg->{'images'}->{'standard_resolution'}->{'url'}; //standard_resolution
                    ...
                    //The markup
                    $simpleDisplay.='<img onClick="instagram_selectImage(' . $i . ', this);" src="'.$istg_thumbnail.'" alt="'.$istg_caption.'" title="'.$istg_caption.'" />';
